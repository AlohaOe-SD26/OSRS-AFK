...
func _spawn_monsters(n: int) -> void:
	...
	if rat == null:
		return
	for i in range(n):
		monsters.append(MonsterInstance.from_type(rat, _combat_scatter()))

func _combat_scatter() -> Vector2:
...
func _set_move(h: Hero, loc_key: String) -> void:
	...
	h.move_target = location_tile(loc_key) + Vector2(rng.randf_range(-spread, spread), rng.randf_range(-spread, spread))
...
func _respawn_monster(r: MonsterInstance) -> void:
	var rat: Monster = content.monster("rat")
	if rat != null:
		var fresh := MonsterInstance.from_type(rat, _combat_scatter())
...
func _gear_drop(h: Hero) -> void:
	...
	if new_tier > cur_tier:
		if h.equipped.has(slot):
			h.gold += float(Config.GEAR_DROPS[0]["value"]) * 0.0 + 8.0   # salvage the old piece (flat scrap)
...
func _fight_round(h: Hero) -> void:
	...
	if h.pos.distance_to(r.pos) > reach:
		h.move_target = r.pos
		return
	# hero attacks — canon rolls (Combat.gd), aggressive style trains Strength
	var eff_att := Combat.effective_level(h.skill_level("attack"), 1.0, 0)
	var eff_str := Combat.effective_level(h.skill_level("strength"), 1.0, 0)
	var acc := Combat.hit_chance(Combat.attack_roll(eff_att, GEAR_ATT_BONUS), Combat.defence_roll(r.defence, 0))
	var mh := Combat.max_hit(eff_str, GEAR_STR_BONUS)
	var dmg := 0
	if rng.chance(acc):
		dmg = rng.randi_range(0, mh)
		r.hp -= dmg
	if dmg > 0:
		_grant_xp(h, "strength", dmg * 4)
		_grant_xp(h, "hitpoints", int(round(dmg * 1.33)))
	# monster retaliates
	if rng.chance(MONSTER_RETALIATE_CHANCE):
		h.hp -= rng.randi_range(0, r.monster_max_hit)
	# kill
	if r.hp <= 0:
		r.alive = false
...
func _maybe_pick_goal(h: Hero) -> void:
	...
	if not h.goal.is_empty():
		if h.skill_level(String(h.goal["skill"])) >= int(h.goal["level"]):
			...
			return
	var pool := ["mining", "woodcutting", "fishing", "cooking", "strength"]
...
func _work_action(h: Hero) -> void:
	...
	match a["phase"]:
		...
		"gather":
			...
			if int(h.inv[a["res"]]) >= 14 or h.cargo_count() >= 27:
				a["phase"] = "goto"
				a["target"] = "shop"
				a["then"] = "sell"
				_set_move(h, "shop")
