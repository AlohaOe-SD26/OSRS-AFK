...
## Statistical projection used by offline catch-up (§3) and the brain's reward estimate.
## Returns { "xp": float, "gold": float, "items": float } expected PER HOUR for this hero
## doing this activity, before the §3 offline rate is applied.
