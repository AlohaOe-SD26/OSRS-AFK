...
static func _build_table() -> PackedInt64Array:
	...
	for n in range(1, MAX_LEVEL):
		points += floor(n + 300.0 * pow(2.0, n / 7.0))
		table[n + 1] = int(floor(points / 4.0))
	return table
...
static func combat_level(att: int, strn: int, def: int, hp: int, ranged: int, magic: int, prayer: int) -> int:
	var base := 0.25 * (def + hp + floor(prayer / 2.0))
	var melee := 0.325 * (att + strn)
	var rng_cb := 0.325 * (floor(ranged * 3.0 / 2.0))
	var mag_cb := 0.325 * (floor(magic * 3.0 / 2.0))
	var best := maxf(melee, maxf(rng_cb, mag_cb))
	return int(floor(base + best))
