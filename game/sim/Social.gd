...
func _proximity_pass(world, elapsed_days: float) -> void:
	...
	for loc in groups:
		...
		if ids.size() < 2:
			continue
		var alive_here: int = world.alive_monster_count_at(loc)   # >0 ⇒ this is a combat camp
		var delta: float
		if alive_here > 0 and ids.size() > alive_here:
			delta = -minf(Config.REL_FRICTION_CAP, Config.REL_FRICTION * elapsed_days)   # contested camp → friction
		else:
			delta = minf(Config.REL_PROX_CAP, Config.REL_PROXIMITY * elapsed_days)        # abundant node → bond
			if alive_here > 0:
				delta += Config.REL_COOP                  # co-op-survive flavor at an uncrowded camp
