...
# claude.exe sometimes lives in %USERPROFILE%\.local\bin without being on PATH
# for this shell - add it defensively before checking:
if (Test-Path "$env:USERPROFILE\.local\bin\claude.exe") { $env:PATH = "$env:USERPROFILE\.local\bin;" + $env:PATH }
